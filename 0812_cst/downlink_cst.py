...
# channel 논문 함수 만들기  ==== 안씀 ㅠㅠㅠㅠㅠ  => GetMinCSThresh쓰기
...
# -----내가 다시 만드는 중 ------------------------------------
def SmartCarrierSenseAP(ap_ind, tx_vector, tx_dest, aps, mss, mem, chs, ch, cst0,cst1,cst2,cst3,  MARGIN=0):
    if tx_vector[ap_ind] == 1:
        return True
...
def RunSim(NUM_GRID=3, NUM_NODES=0, NODES_PER_CELL=1, AREA_SIZE=10, NUM_CH=1, CS=-99, PROTO=0, SEED=0,
    ...
    # print("거리기준=", distlis[divind_dist-1], distlis[divind_dist*2-1], distlis[divind_dist*3-1], distlis[param_num_nodes-1]) #채널기준들
    ...
    min_ch3_dist = distlis[param_num_nodes-1] #나머지처리
            ...
            while found:
                ...
                for i in range(param_num_aps):
                    ...
                    if param_protocol == 0:
                        ...
                        cs = SmartCarrierSenseAP(i, tx_vector, dst[:, ch], aps, mss, mem, chs, ch, cst0,cst1,cst2,cst3, param_margin)
