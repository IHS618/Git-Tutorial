...
# new SmartCarrierSenseAP--------------------------------------------------------------------------------
def SmartCarrierSenseAP(ap_ind, tx_vector, tx_dest, aps, mss, mem, chs, ch, cst0,cst1,cst2,cst3,
                        cst4,cst5,cst6,cst7,cst8, MARGIN=0):
    if tx_vector[ap_ind] == 1:
        return True
    ...
    for i in range(len(aps)):
        ...
        if tx_vector[i] == 1:
            ...
            for j in range(len(mss)): # ms수
                if mem[ap_ind,j]==1 and chs[j,ch]==1 : #i를 ap_ind로
                    
                    if  ch==0 and chs[j,0]==1 :
                        if ilevel > GetMinCSThresh(GetDist(aps[i], mss[tx_dest[i]])) - MARGIN:
                            return True
                        else:
                            total_interference_w += DB2W(ilevel)
    ...
    for j in range(len(mss)): #ms 수          
        if mem[ap_ind,j]==1 and chs[j,ch]==1 : #연결 #and dst[i,ch]==j:
            ...
            elif ch==1 and chs[j,1]==1 :  #채널1     
                if(total_interference_db > cst1):
                    return True  # busy
                else:
                    return False  # idle
            
            elif ch==2 and chs[j,2]==1 :  #채널2    
                if(total_interference_db > cst2):
                    return True  # busy
                else:
                    return False  # idle
            
            elif ch==3 and chs[j,3]==1 :  #채널3    
            ...
            # 추가 -----
            elif ch==4 and chs[j,4]==1 :  #채널4   
                if(total_interference_db > cst4):
                    return True  # busy
                else:
                    return False  # idle
                
            elif ch==5 and chs[j,5]==1 :  #채널5    
                if(total_interference_db > cst5):
                    return True  # busy
                else:
                    return False  # idle
            
            elif ch==6 and chs[j,6]==1 :  #채널6   
                if(total_interference_db > cst6):
                    return True  # busy
                else:
                    return False  # idle
                
            elif  ch==7 and chs[j,7]==1 :  #채널7    
                if(total_interference_db > cst7):
                    return True  # busy
                else:
                    return False  # idle 
                
            elif ch==8 and chs[j,8]==1 :  #채널8   
                if(total_interference_db > cst8):
                    return True  # busy
                else:
                    return False  # idle
...
def RunSim(NUM_GRID=3, NUM_NODES=0, NODES_PER_CELL=1, AREA_SIZE=10, NUM_CH=1, CS=-99, PROTO=0, SEED=0,
    ...
    # 채널별 가장 거리가 긴 노드 = 채널별 RSSI가 작은 노드 ------------------------adding
    min_ch0_dist=1000
    min_ch1_dist=1000
    min_ch2_dist=1000
    min_ch3_dist=1000
    ...
    while True:
        min_ch0_dist = dislis[nic[0]-1]  
        if param_num_channels ==1:
                break
        min_ch1_dist = dislis[nic[0]+nic[1]-1]
        if param_num_channels ==2:
                break
        min_ch2_dist = dislis[nic[0]+nic[1]+nic[2]-1]
        if param_num_channels ==3:
                break
        min_ch3_dist = dislis[nic[0]+nic[1]+nic[2]+nic[3]-1]
        if param_num_channels ==4:
                break
        
        # 추가 ---- 
        min_ch4_dist = dislis[nic[0]+nic[1]+nic[2]+nic[3]+nic[4]-1]
        if param_num_channels ==5:
                break
        min_ch5_dist = dislis[nic[0]+nic[1]+nic[2]+nic[3]+nic[4]+nic[5]-1]
        if param_num_channels ==6:
                break
        min_ch6_dist = dislis[nic[0]+nic[1]+nic[2]+nic[3]+nic[4]+nic[5]+nic[6]-1]
        if param_num_channels ==7:
                break
        min_ch7_dist = dislis[nic[0]+nic[1]+nic[2]+nic[3]+ nic[4]+nic[5]+nic[6]+nic[7]-1]
        if param_num_channels ==8:
                break
        min_ch8_dist = dislis[nic[0]+nic[1]+nic[2]+nic[3]+nic[4]+nic[5]+nic[6]+nic[7]+nic[8]-1]
        if param_num_channels ==9:
                break
    ...
    # 추가 ----
    cst4 = GetMinCSThresh(min_ch4_dist)
    cst5 = GetMinCSThresh(min_ch5_dist)
    cst6 = GetMinCSThresh(min_ch6_dist)
    cst7 = GetMinCSThresh(min_ch7_dist)
    cst8 = GetMinCSThresh(min_ch8_dist)
    ...
    if param_verbose == True:
        print('----------------------------------')
        print(' simulation area     : %dx%d' % (param_num_grid, param_num_grid))
        if NUM_NODES == 0:
            ...
            while found:
                ...
                for i in range(param_num_aps):
                    ...
                    if param_protocol == 0:
                        ...
                        cs = SmartCarrierSenseAP(i, tx_vector, dst[:, ch], aps, mss, mem, chs, ch, cst0,cst1,cst2,cst3, 
                                                 cst4,cst5,cst6,cst7,cst8, param_margin)
