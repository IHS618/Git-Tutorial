#!/usr/bin/env python
import numpy as np
from downlink_pro import RunSim
import sys
...
#                                           END=end, VERBOSE=True, IDEAL=False, LATEX=False, MARGIN=m, newmargin=newmargin)
...
#                 print("%5d %5d %5d %5d %5d %8.2f %5d %8.2f %8.2f %8.2f %8.2f %8.2f %8.2f %8.2f %8.2f"%(g, a, n, c, p, cs, m, s, t, ht, lt, f, tx, rx, newmargin), file=fn)
...
# --------------------------------------------------
# newmargin
# for newmargin in np.arange(0.1, 1.1, 0.1): # 0.1 - 0.9
#     a = 300/g
#     for p in range(0, 1):
#         for s in range(1000, 1010):
#             t, ht, lt, f, tx, rx = RunSim(NUM_GRID=g, AREA_SIZE=a, NUM_NODES=n, NUM_CH=c, PROTO=p, CS=cs, SEED=s,
...
#             with open('result_margin_300.txt', 'a') as fn:   
#                 print("%5d %5d %5d %5d %5d %8.2f %5d %8.2f %8.2f %8.2f %8.2f %8.2f %8.2f %8.2f %8.2f"%(g, a, n, c, p, cs, m, s, t, ht, lt, f, tx, rx, newmargin), file=fn)
...
#             # fn = str('result_g%02d_n%03d_c%02d_p%01d_cs%03d_m%02d_s%05d_a%03d.txt' %(g, n, c, p, -cs, m, s, a))
#             # np.savetxt(fn, (g, a, n, c, p, cs, m, s, t, ht, lt, f, tx, rx), fmt='%.6f')
#             with open('result_area_300_20.txt', 'a') as fn:   
#                 print("%5d %5d %5d %5d %5d %8.2f %5d %8.2f %8.2f %8.2f %8.2f %8.2f %8.2f %8.2f"%(g, a, n, c, p, cs, m, s, t, ht, lt, f, tx, rx), file=fn)   


# 연습용

# for g in range(10, 14, 100): # 3*3 - 13*13 일때
#     a = 100/g
#     for p in range(0, 1):
#         for s in range(1000, 1001, 1000):
#             t, ht, lt, f, tx, rx = RunSim(NUM_GRID=g, AREA_SIZE=a, NUM_NODES=n, NUM_CH=c, PROTO=p, CS=cs, SEED=s,
#                                           END=end, VERBOSE=True, IDEAL=False, LATEX=False, MARGIN=m, newmargin=newmargin)
#             # fn = str('result_g%02d_n%03d_c%02d_p%01d_cs%03d_m%02d_s%05d.txt' %(g, n, c, p, -cs, m, s))
#             # np.savetxt(fn, (g, a, n, c, p, cs, m, s, t, ht, lt, f, tx, rx), fmt='%.6f') 
#             with open('result_연습.txt', 'a') as fn:   
#                 print("%5d %5d %5d %5d %5d %8.2f %5d %8.2f %8.2f %8.2f %8.2f %8.2f %8.2f %8.2f %8.2f"%(g, a, n, c, p, cs, m, s, t, ht, lt, f, tx, rx, newmargin), file=fn)
