...
#+++++++++++++++adding+ 채널마다 마진을 기준으로 tx power 조절+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
...
def GetRxPowerDB(a, b): 
    for p in range(len(chs)):
        ...
        ch0_min_margin0, ch1_min_margin1, ch2_min_margin2, ch3_min_margin3 = ListMarginPerChannel()               
       
        #margin이 0보다 작은게 있으면 tx = 20 아니면 tx를 다르게 할당
        if chs[p,0] == 1 :   #ch==0
            return PxControl (ch0_min_margin0, a, b)                                       
        elif chs[p,1] == 1:  #ch == 1
            return PxControl (ch1_min_margin1, a, b)                                  
        elif chs[p,2] == 1:  #ch == 2
            return PxControl (ch2_min_margin2, a, b)                    
        elif chs[p,3] == 1:  #ch == 3
            return PxControl (ch3_min_margin3, a, b)                    
        else :
            return print("??")
...
# get margin 각 노드의 마진 구하기 ---------------------수정중 
   
def GetMargin (aa, bb):  # a,b for문에서 i(ap), j(node)
    # margin_vector = []           
    if mem[aa,bb]==1:
        memind=[aa,bb]     #1인 index
        #print("함수안 memind", memind)
        
        GetRxPowerDB_Original(aps[i], mss[j])
        #print("rx파워", GetRxPowerDB_Original(aps[i], mss[j]))   #rx파워랑 signal_db 신호세기랑 같은 값이 나옴...
        
        my_snr_db= GetRxPowerDB_Original(aps[i], mss[j]) - NOISE_FLOOR
        #print("my_snr_db", my_snr_db)     # snr_db랑 my_snr_db랑 같은 값이 나옴....
        
        my_margin  = my_snr_db - SNR_THRESHOLD
...
#채널별 최소 마진 구하기------------------------------
def ListMarginPerChannel():   # a,b for문에서 i(ap), j(node) / c는 채널
    ch0_margin = []
    ch1_margin = []
    ch2_margin = []
    ch3_margin = []     
    ...
    return ch0_min_margin, ch1_min_margin, ch2_min_margin, ch3_min_margin
...
print("0000000000000000000000", len(chs))
...
print("채널별 마진 리스트로 구한 후 min 최소값 구하기")              
qq,ww,ee,rr =ListMarginPerChannel ()                               
print (qq,ww,ee,rr)                  
