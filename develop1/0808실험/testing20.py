...
def GetRxPowerDB_Original(a, b):   # 이걸 사용해야하나 Rssi로 ???
    return REFERENCE_TX_POWER - GetPathLossDB(GetDist(a, b))

def GetRxPowerDB_Model_Original(a, b):  #CST????
    return REFERENCE_TX_POWER - GetPathLossDB_Model(GetDist(a, b))


#+++++++++++++++adding+ 채널마다 tx power 조절+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
def PxControl (m, a, b):
    if m < 30 : 
        return 20 - GetPathLossDB(GetDist(a, b))       
    elif 30 <= m < 40 :   #ch == 1
        return 20 - GetPathLossDB(GetDist(a, b))    
    elif 40 <= m < 50 :  #ch == 2
        return 20 - GetPathLossDB(GetDist(a, b))   
...
def GetRxPowerDB(a, b, ch0_min_margin, ch1_min_margin, ch2_min_margin, ch3_min_margin): 
    for c in range(len(chs)):
        #채널0에서 모든 margin중 가장 작은 값 가져오기
        
        #margin이 0보다 작은게 있으면 tx = 20 아니면 tx를 다르게 할당
        if chs[c,0] == 1 :   #ch==0
...
#         if chs[i,0] == 1 :  #ch == 0
#             return 10 - GetPathLossDB(GetDist(a, b))
        
#         elif chs[i,1] == 1:  #ch == 1
#             return 13 - GetPathLossDB(GetDist(a, b))
        
#         elif chs[i,2] == 1:  #ch == 2
#             return 16 - GetPathLossDB(GetDist(a, b))
...
# def GetRxPowerDB_Model(chs, a, b): 
#     for i in range(len(chs)):
        
#         if chs[i,0] == 1 :  #ch == 0
#             return 10 - GetPathLossDB_Model(GetDist(a, b))
        
#         elif chs[i,1] == 1:  #ch == 1
#             return 13 - GetPathLossDB_Model(GetDist(a, b))
        
#         elif chs[i,2] == 1:  #ch == 2
#             return 16 - GetPathLossDB_Model(GetDist(a, b))
        
#         elif chs[i,3] == 1:  #ch == 3
#             return 20 - GetPathLossDB_Model(GetDist(a, b))
...
# get margin ---------------------수정중    
def GetMargin (a, b):  # a,b for문에서 i(ap), j(node)
    # margin_vector = []           
    if mem[a,b]==1:
        memind=[a,b]     #1인 index
        ...
        # print("signal_db 신호세기 = rx파워", signal_db)     #---------------------------해제
        
        total_noise_w = DB2W(NOISE_FLOOR)
        #print("total_noise_w 노이즈", total_noise_w)
        ...
        # print("snr_db 노드 snr", snr_db)            #---------------------------해제
        
        my_margin  = snr_db - SNR_THRESHOLD     # mem[i,j]의 margin
        print("함수안 my_margin = ", my_margin)
        
        # margin_vector.append(my_margin)       #마진 더해서 벡터에 넣기
        # margin_vector = sorted(margin_vector)  #정렬한 이유는 나도 모름  
...
# def ListMarginPerChannel (a, b):   # a,b for문에서 i(ap), j(node) / c는 채널
#     ch0_margin=[]
#     ch1_margin=[]
#     ch2_margin=[]
#     ch3_margin=[]  
#     for k in range (4): #채널 4개             
#         if mem[a,b]==1 and chs[k,0]==1: #ch0
#             #for m in range (int(param_num_nodes/4)):
#             ch0_margin = ch0_margin.append(GetMargin(i, j))
            
#         elif mem[a,b]==1 and chs[k,1]==1: #ch1
#              #for m in range (int(param_num_nodes/4)):
#             ch1_margin = ch1_margin.append(GetMargin(i, j))
                                                  
#         elif mem[a,b]==1 and chs[k,2]==1: #ch2
#              #for m in range (int(param_num_nodes/4)):
#             ch2_margin = ch2_margin.append(GetMargin(i, j))                       
                
#         elif mem[a,b]==1 and chs[k,3]==1: #ch3
#              #for m in range (int(param_num_nodes/4)):
#             ch3_margin = ch3_margin.append(GetMargin(i, j))
                              
#         else:
#             print("error ListMarginPerChannel") 
            
#         return ch0_margin, ch1_margin, ch2_margin, ch3_margin
...
def CarrierSenseAP(ap_ind, tx_vector, aps, threshold=DEFAULT_CS_THRESHOLD):  
    ...
    for i in range(len(aps)):
        if tx_vector[i] == 1:
            total_interference_w += DB2W(GetRxPowerDB(aps[i], aps[ap_ind], ch0_min_margin, ch1_min_margin, ch2_min_margin, ch3_min_margin))  #---------------ch
...
def DSCCarrierSenseAP(ap_ind, tx_vector, tx_dest, aps, mss, MARGIN=0):
    ...
    for i in range(len(aps)):
        if tx_vector[i] == 1:
            total_interference_w += DB2W(GetRxPowerDB(aps[i], aps[ap_ind])) #--------------------ch
    if total_interference_w == 0.0:
        return False
    total_interference_db = W2DB(total_interference_w)
...
def SmartCarrierSenseAP(ap_ind, tx_vector, tx_dest, aps, mss, MARGIN=0):
    ...
    for i in range(len(aps)):
        if i == ap_ind:
            continue
        if tx_vector[i] == 1:
            ilevel = GetRxPowerDB(aps[i], aps[ap_ind]) #----------------------------ch
...
def NewCarrierSenseAP(ap_ind, tx_vector, tx_dest, aps, mss,   MARGIN=0):
    ...
    for i in range(len(aps)):
        if i == ap_ind:
            continue
        if tx_vector[i] == 1:
            ilevel = GetRxPowerDB(aps[i], mss[tx_dest[ap_ind]]) #----------------------ch
            slevel = GetRxPowerDB(aps[ap_ind], mss[tx_dest[ap_ind]])  #----------------------ch
    ...
    for i in range(len(aps)):
        if i == ap_ind:
            continue
        if tx_vector[i] == 1:
            ilevel = GetRxPowerDB(aps[ap_ind], mss[tx_dest[i]])  #----------------------ch
            slevel = GetRxPowerDB(aps[i], mss[tx_dest[i]])  #----------------------ch
            if slevel - ilevel < SNR_THRESHOLD + MARGIN:
                return True
...
def TransmitAP(tx_vector, tx_dest, mss, aps):
    ...
    for i in range(len(aps)):
        if tx_vector[i] == 1:
            ...
            signal_db = GetRxPowerDB(aps[i], mss[tx_dest[i]], ch0_min_margin, ch1_min_margin, ch2_min_margin, ch3_min_margin) #---------------------ch
            total_noise_w = DB2W(NOISE_FLOOR)
            # calculate interference
            for j in range(len(aps)):
                if i == j or tx_vector[j] == 0:
                    continue
                total_noise_w += DB2W(GetRxPowerDB(aps[j], mss[tx_dest[i]], ch0_min_margin, ch1_min_margin, ch2_min_margin, ch3_min_margin))  #---------------------ch
    ...
    for i in range(param_num_aps):  
        for j in range(param_num_nodes):               
            if mem[i,j]==1:
                ...
                dislis = sorted(dislis)     #거리 배열 정렬 오름차순
# print("dislis", dislis) 
# print("divind", divind)    
...
print("vvvvvvvvvvvvvvvvvvvvvv플리즈 마진 최소 나와라 ㅠㅠㅠㅠㅠㅠㅠㅠ")
ch0_margin = []
ch1_margin = []
ch2_margin = []
ch3_margin = []  
    ...
    for i in range(param_num_aps):  
        for j in range(param_num_nodes):
            for k in range (param_num_channels):
                ...
                if mem[i,j]==1 and chs[j,k]==1:
                    
                    if  dst[i,0]>-1 and chs[j,0]==1 : #ch0 
                        print("채널마진GetMargin(i, j)=====", GetMargin(i, j))
                        ch0_margin.append(GetMargin(i, j))
                        print("채널0에서 margin list", ch0_margin)
                        print("=========")
                        
                    elif dst[i,1]>-1 and chs[j,1]==1: #ch1 
                        print("채널마진GetMargin(i, j)=====", GetMargin(i, j))                   
                        ch1_margin.append(GetMargin(i, j))
                        print("채널1에서 margin list", ch0_margin)
                        print("=========")
                                                              
                    elif dst[i,2]-1 and chs[j,2]==1: #ch2 
                        print("채널마진GetMargin(i, j)=====", GetMargin(i, j))                    
                        ch2_margin.append(GetMargin(i, j))
                        print("채널2에서 margin list", ch0_margin)
                        print("=========")
                            
                    elif dst[i,3]>-1 and chs[j,3]==1: #ch3  
                        print("채널마진GetMargin(i, j)======", GetMargin(i, j))                  
                        ch3_margin.append(GetMargin(i, j))
                        print("채널3에서 margin list", ch0_margin)
                        print("=========")
...
#채널별 최소 마진 출력-------------------------
print("채널0 최소 마진", ch0_min_margin)
print("채널1 최소 마진", ch1_min_margin)
print("채널2 최소 마진", ch2_min_margin)
print("채널3 최소 마진", ch3_min_margin)
