...
def GetRxPowerDB(a, b):   # 이걸 사용해야하나 Rssi로 ???
    return REFERENCE_TX_POWER - GetPathLossDB(GetDist(a, b))

def GetRxPowerDB_Model(a, b):  #CST????
...
def TransmitAP(tx_vector, tx_dest, mss, aps):
    ...
    for i in range(len(aps)):
        if tx_vector[i] == 1:
            ...
            signal_db = GetRxPowerDB(aps[i], mss[tx_dest[i]]) #---------------------ch
            total_noise_w = DB2W(NOISE_FLOOR)
            # calculate interference
            for j in range(len(aps)):
                if i == j or tx_vector[j] == 0:
                    continue
                total_noise_w += DB2W(GetRxPowerDB(aps[j], mss[tx_dest[i]]))  #---------------------ch
...
print("거리기준=", distlis[divind_dist-1], distlis[divind_dist*2-1], distlis[divind_dist*3-1], distlis[divind_dist*4-1]) #채널기준들
...
Ds = GetDistFromPL(REFERENCE_TX_POWER - Ps0)
print("Ds", Ds) 

# Di 구하기  ------------
Di = GetDistFromPL(REFERENCE_TX_POWER - (Ps0 - SNR_THRESHOLD))
print("Di", Di) 
...
cst1 = CSTperChannel(Ps0)
cst2 = CSTperChannel(Ps0)
cst3 = CSTperChannel(Ps0)
