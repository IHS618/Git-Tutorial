...
def PxControl (min_margin, a, b):   
    if min_margin > 0:
        # print("함수 min_margin",a,b, min_margin)
        return REFERENCE_TX_POWER - GetPathLossDB(GetDist(a, b)) - (min_margin*0.5)
    else :
        return REFERENCE_TX_POWER - GetPathLossDB(GetDist(a, b))
    

# new GetRxPowerDB -------------------------------------
def GetRxPowerDB(a, b, ch, i, ch0_min_margin, ch1_min_margin, ch2_min_margin, ch3_min_margin): #a=ap[위치 50, 20 ]
    for j in range(len(mss)): # mms수 
        if mem[i,j]==1 and chs[j,ch]==1 and dst[i,ch]==j: #연결
...
def CarrierSenseAP(ap_ind, tx_vector, aps,ch, ch0_min_margin, ch1_min_margin, ch2_min_margin, ch3_min_margin, threshold=DEFAULT_CS_THRESHOLD):  
    if tx_vector[ap_ind] == 1:
        return True
    total_interference_w = 0.0
    for i in range(len(aps)):
        if tx_vector[i] == 1:
            total_interference_w += DB2W(GetRxPowerDB(aps[i], aps[ap_ind],ch, i, ch0_min_margin, ch1_min_margin, ch2_min_margin, ch3_min_margin))
...
def TransmitAP(tx_vector, tx_dest, mss, aps, ch, ch0_min_margin, ch1_min_margin, ch2_min_margin, ch3_min_margin):
    rx_vector = np.copy(tx_vector)
    rx_snr = np.zeros(tx_vector.shape)
    for i in range(len(aps)):
        if tx_vector[i] == 1:
            # AP i is transmitting to tx_dest[i]
            signal_db = GetRxPowerDB(aps[i], mss[tx_dest[i]], ch,i, ch0_min_margin, ch1_min_margin, ch2_min_margin, ch3_min_margin)
            total_noise_w = DB2W(NOISE_FLOOR)
            # calculate interference
            for j in range(len(aps)):
                if i == j or tx_vector[j] == 0:
                    continue
                total_noise_w += DB2W(GetRxPowerDB(aps[j], mss[tx_dest[i]], ch,i, ch0_min_margin, ch1_min_margin, ch2_min_margin, ch3_min_margin))
    ...
    for i in range(param_num_aps):  
        for j in range(param_num_nodes):
            for k in range (param_num_channels):
                ...
                if mem[i,j]==1 and chs[j,k]==1 and dst[i,k]==j: 
...
for t in range(param_end_time):
    ...
    for ch in range(param_num_channels):
        ...
        while found:
            ...
            for i in range(param_num_aps):
                if tx_vector[i] == 1 or blocked[i] == 1:
                    continue
                if param_protocol == 0:
                    cs = CarrierSenseAP(i, tx_vector, aps, ch, ch0_min_margin, ch1_min_margin, ch2_min_margin, ch3_min_margin, threshold=cst[ch])
        ...
        # transmit packets -------------------------------------------------------
        rx_vector, _ = TransmitAP(tx_vector, dst[:, ch], mss, aps, ch, ch0_min_margin, ch1_min_margin, ch2_min_margin, ch3_min_margin)
