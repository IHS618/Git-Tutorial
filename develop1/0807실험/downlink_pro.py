...
def PxControl (mar, a, b):
    if mar < 20 : 
        ...
        return 5 - GetPathLossDB(GetDist(a, b)) 
...
# new GetRxPowerDB -------------------------------------
def GetRxPowerDB(a, b, chs, ch0_min_margin, ch1_min_margin, ch2_min_margin, ch3_min_margin): 
...
def CarrierSenseAP(ap_ind, tx_vector, aps, chs, ch0_min_margin, ch1_min_margin, ch2_min_margin, ch3_min_margin, threshold=DEFAULT_CS_THRESHOLD):  
    if tx_vector[ap_ind] == 1:
        return True
    total_interference_w = 0.0
    for i in range(len(aps)):
        if tx_vector[i] == 1:
            total_interference_w += DB2W(GetRxPowerDB(aps[i], aps[ap_ind], chs, ch0_min_margin, ch1_min_margin, ch2_min_margin, ch3_min_margin))
...
def TransmitAP(tx_vector, tx_dest, mss, aps, chs, ch0_min_margin, ch1_min_margin, ch2_min_margin, ch3_min_margin):
    rx_vector = np.copy(tx_vector)
    rx_snr = np.zeros(tx_vector.shape)
    for i in range(len(aps)):
        if tx_vector[i] == 1:
            # AP i is transmitting to tx_dest[i]
            signal_db = GetRxPowerDB(aps[i], mss[tx_dest[i]], chs, ch0_min_margin, ch1_min_margin, ch2_min_margin, ch3_min_margin)
            total_noise_w = DB2W(NOISE_FLOOR)
            # calculate interference
            for j in range(len(aps)):
                if i == j or tx_vector[j] == 0:
                    continue
                total_noise_w += DB2W(GetRxPowerDB(aps[j], mss[tx_dest[i]], chs, ch0_min_margin, ch1_min_margin, ch2_min_margin, ch3_min_margin))
...
def RunSim(NUM_GRID=3, NUM_NODES=0, NODES_PER_CELL=1, AREA_SIZE=10, NUM_CH=1, CS=-99, PROTO=0, SEED=0,
                ...
                for k in range (param_num_channels):
                    ...
                    if mem[i,j]==1 and chs[j,k]==1:
                        ...
                        if  dst[i,0]>-1 and chs[j,0]==1 : #ch0 
                            print("채널마진GetMargin(i, j)=====", GetMargin(i, j,  mem, aps, mss))
    ...
    for t in range(param_end_time):
        ...
        for ch in range(param_num_channels):
            ...
            while found:
                ...
                for i in range(param_num_aps):
                    if tx_vector[i] == 1 or blocked[i] == 1:
                        continue
                    if param_protocol == 0:
                        cs = CarrierSenseAP(i, tx_vector, aps, chs, ch0_min_margin, ch1_min_margin, ch2_min_margin, ch3_min_margin, threshold=cst[ch])
            ...
            # transmit packets -------------------------------------------------------
            rx_vector, _ = TransmitAP(tx_vector, dst[:, ch], mss, aps, chs, ch0_min_margin, ch1_min_margin, ch2_min_margin, ch3_min_margin)
