...
def PxControl (min_margin, a, b):   
    if min_margin > 0 :
        return REFERENCE_TX_POWER - GetPathLossDB(GetDist(a, b)) -  ( min_margin*0.4 )  
    else :
        return REFERENCE_TX_POWER - GetPathLossDB(GetDist(a, b)) 
    

# new GetRxPowerDB -------------------------------------
def GetRxPowerDB(a, b, ch, ap_ind, mss, mem, chs, ch0_min_margin, ch1_min_margin, ch2_min_margin, ch3_min_margin): #a=ap[위치 50, 20 ]
...
# 내가 다시만듬 변경 -----------------------
def SmartCarrierSenseAP(ap_ind, tx_vector, tx_dest, aps, mss, ch, mem, chs, ch0_min_margin, ch1_min_margin, ch2_min_margin, ch3_min_margin, MARGIN=0, threshold=DEFAULT_CS_THRESHOLD ):
    if tx_vector[ap_ind] == 1:
        return True
...
# 내가 다시 만듬 -----------------------------------------------
def NewCarrierSenseAP(ap_ind, tx_vector, tx_dest, aps, mss, ch, mem, chs, ch0_min_margin, ch1_min_margin, ch2_min_margin, ch3_min_margin,  MARGIN=0):
    ...
    for i in range(len(aps)):
        if i == ap_ind:
            continue
        if tx_vector[i] == 1:
            ilevel = GetRxPowerDB(aps[ap_ind], mss[tx_dest[i]],ch, ap_ind, mss, mem, chs, ch0_min_margin, ch1_min_margin, ch2_min_margin, ch3_min_margin)
            slevel = GetRxPowerDB(aps[i], mss[tx_dest[i]],ch, i, mss, mem, chs, ch0_min_margin, ch1_min_margin, ch2_min_margin, ch3_min_margin)
            # 위 ap_ind를 i로
            if slevel - ilevel < SNR_THRESHOLD + MARGIN:
                return True
...
def RunSim(NUM_GRID=3, NUM_NODES=0, NODES_PER_CELL=1, AREA_SIZE=10, NUM_CH=1, CS=-99, PROTO=0, SEED=0,
        ...
        for i in range(param_num_aps):  
            for j in range(param_num_nodes):               
                if mem[i,j]==1:
                    ...
                    elif mydistance >= dislis[divind*4-1]:  #나머지 오류처리
                        ch=3
                        #print("ch==3", ch)
    ...
    #채널별 마진 리스트 중 가장 작은 값------------ 사용안함
    # if ch0_margin==[] or ch1_margin==[] or ch2_margin==[] or ch3_margin==[]: #area =0 오류때문에 추가
    #     ch0_min_margin=0
    #     ch1_min_margin=0
    #     ch2_min_margin=0
    #     ch3_min_margin=0
            ...
            while found:
                ...
                for i in range(param_num_aps):
                    ...
                    if param_protocol == 0:
                        ...
                        cs = SmartCarrierSenseAP(i, tx_vector, dst[:, ch], aps, mss, ch, mem, chs, ch0_min_margin, ch1_min_margin, ch2_min_margin, ch3_min_margin, param_margin, threshold=cst[ch])
                    elif param_protocol >= 3:
                        cs = NewCarrierSenseAP(i, tx_vector, dst[:, ch], aps, mss, ch, mem, chs, ch0_min_margin, ch1_min_margin, ch2_min_margin, ch3_min_margin, param_margin)
                    if cs == True:
