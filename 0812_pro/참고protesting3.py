...
def PxControl (min_margin, a, b):   
    if min_margin > 0:
        return REFERENCE_TX_POWER - GetPathLossDB(GetDist(a, b)) - (min_margin* min_margin/44.2923 ) #생각해봐야함
    else :
        return REFERENCE_TX_POWER - GetPathLossDB(GetDist(a, b))
    ...
    for i in range(param_num_aps):  
        for j in range(param_num_nodes):               
            if mem[i,j]==1:
                ...
                elif mydistance >= dislis[divind*4-1]:  #에러때문에 추가
                    ch=3
                    print("ch==3", ch)
...
# +추가 다음번호 채널의 가장 큰 마진값만큼 뺴주면 어떨까..?
# 1. 채널별 가장 큰 마진을 찾는다.
# 2. 0번채널은 1번채널의 가장 큰 마진으로 빼준다/ 채널1,2, 동일
# 3. 3번 채널은 가장작은 마진보다 기준 크기 1만큼의 마진을 빼서 빼준다. 
